...
function resizeCanvas(){const canvas = document.getElementById("gameCanvas");
const ctx = canvas.getContext("2d");
const scoreElement = document.getElementById("score");
const timerElement = document.getElementById("timer");
...
function generateSectors(){
    ...
    const gap = 15;
    const startY = 20;
    const x = canvas.width - 260; // Afastado um pouco mais para a esquerda devido ao tamanho dinâmico
    return [
...
function createPlant() {
    ...
    // Configura o tamanho padrão inicial da planta
    let plantWidth = 140;
    const plantHeight = 55;

    // Mede o tamanho do texto do problema para ver se vai estourar a caixinha
    ctx.font = "bold italic 12px Arial";
    const textWidth = ctx.measureText(`Problema: ${disease.name}`).width;
    
    // Se o texto precisar de mais espaço que o padrão (+ margens), a caixinha aumenta!
...
// Desenha setores do lado direito (Com largura adaptada para nomes grandes)
function drawSectors(){
    sectors.forEach(sector=>{
        ctx.font = "bold 15px Arial";
        const textWidth = ctx.measureText(sector.name).width;
        // Garante que o setor tenha espaço para o texto + círculo + margens (mínimo de 240px)
        ...
        // Guarda a largura real desenhada para detectar a colisão do arrastar perfeitamente
        sector.actualWidth = sectorWidth;
    });
}

// Desenha a planta atual com largura dinâmica calculada
function drawPlants(){
...
// Evento Mouseup com colisão inteligente usando as larguras adaptadas
canvas.addEventListener("mouseup", ()=>{
    ...
    if (acertou) {
        ...
        // Se errar, volta para a esquerda de forma segura
